...
import { make, register } from 'src/Core'

import RuleContract from 'src/Rules/IRuleContract'

const fetchData = (response: any) => new Promise(resolve => setTimeout(() => resolve(response), 1))

describe('Register asyncronous custom rule', function () {
    register('fetch_data', value => fetchData(value) as never)
    const rules = {
...
describe('Register asyncronous custom class rule', function () {
    class FetchData extends RuleContract {
        async passes (value: any) {
            return fetchData(value) as never
        }
...
describe('Defining an asyncronous custom closure rule', function () {
    const rules = {
        test: ['required', async function (value: any, fail: any) {
