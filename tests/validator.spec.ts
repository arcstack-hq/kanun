...
describe('Validator', () => {
    ...
    describe('extended rules', () => {
            ...
            exists ({ table, column, value, ignore }: { table: string, column: string, value: any, ignore?: any }) {
                const records: Record<string, Array<Record<string, any>>> = {
                    users: [
                        ...
                        { id: 2, username: 'kaylah', email: 'kaylah@example.com' },
                    ]
                }

                return (records[table] || []).some((row) => {
                    const ignored = ignore !== undefined && String(row.id) === String(ignore)
        ...
        it('exists: should support ignore values', async () => {
            const v = new Validator(
                { username: 'legacy' },
                { username: 'exists:users,username,1' }
            ).database(driver)
        ...
        it('unique: should support ignore values', async () => {
            const v = new Validator(
                { username: 'legacy' },
