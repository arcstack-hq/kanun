import { Password, make, register } from 'src/Core'
...
describe('Set Errors', function () {
    describe('User can set his own errors by passing an object', function () {
        const validator = make()
        ...
        it('User should be able to set a message for different attributes', function () {
            validator.validate()
...
describe('Append Errors', function () {
    describe('User can append his own errors by passing an object', function () {
        ...
        it('Errors that are being appended explicitly should have a custom error type', function () {
            ...
            const { name, email } = validator.errors().addErrorTypes().all(false)
            assert.equal(email.error_type, 'custom')
            assert.equal(name.error_type, 'string')
        })
    })
})
...
describe('Clear Errors', function () {
    describe('Should remove errors from list', function () {
        const validator = make()
            .setData({})
            .setRules({
                first: 'required',
        ...
        it('If no keys are specified all errors shold be removed', function () {
            validator.validate()
            assert.equal(validator.errors().keys().length, 3)

            validator.clearErrors()
            assert.equal(validator.errors().keys.length, 0)
        })
        it('should only remove the specified keys', function () {
            validator.validate()
            assert.equal(validator.errors().keys().length, 3)
