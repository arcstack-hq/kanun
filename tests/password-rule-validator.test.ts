import { Password, make } from 'src/Core'
...
describe('Password Validation', function () {
    ...
    describe('Custom rules can be passed to the password validation', function () {
        class CustomSymbol extends RuleContract {
            passes (value: any) {
                return /\*/.test(value)
            }
    ...
    describe('Translations can be  added for the password rules', function () {
        ...
        assert.equal(newValidator.errors().first(), 'The password must contain at least 3 letters.')
    })
})